...
func (f *File) ParseRebase(r *bytes.Reader) ([]types.Rebase, error) {
	...
	for !isDone {
		...
		opcode := byte & types.REBASE_OPCODE_MASK
		immediate := byte & types.REBASE_IMMEDIATE_MASK
		switch opcode {
		case types.REBASE_OPCODE_DONE:
			isDone = true
		case types.REBASE_OPCODE_SET_TYPE_IMM:
			opType = uint32(immediate)
		case types.REBASE_OPCODE_SET_SEGMENT_AND_OFFSET_ULEB:
			segmentIndex := uint32(immediate)
			if offset, err := trie.ReadUleb128(r); err != nil {
				log.Panicf("io failed %v", err)
			} else {
				lastSegment = f.Segments()[segmentIndex]
				address = lastSegment.Addr + offset
			}
		...
		case types.REBASE_OPCODE_ADD_ADDR_IMM_SCALED:
			scale := uint32(immediate)
			address += uint64(scale * uint32(ptrSize))
		case types.REBASE_OPCODE_DO_REBASE_IMM_TIMES:
			count := uint32(immediate)
			for i := uint32(0); i < count; i++ {
				var rebase types.Rebase
				rebase.Segment = lastSegment.Name
				rebase.Section = f.FindSectionForVMAddr(address).Name
				rebase.Start = lastSegment.Addr
				rebase.Offset = address - lastSegment.Addr
				rebase.Type = uint8(opType)
				f.cr.SeekToAddr(address)
				binary.Read(f.cr, f.ByteOrder, &rebase.Value)
				rebases = append(rebases, rebase)
				address += ptrSize
			}
		case types.REBASE_OPCODE_DO_REBASE_ULEB_TIMES:
			if count, err := trie.ReadUleb128(r); err != nil {
				log.Panicf("io failed%v", err)
			} else {
				for i := uint64(0); i < count; i++ {
					var rebase types.Rebase
		...
		case types.REBASE_OPCODE_DO_REBASE_ULEB_TIMES_SKIPPING_ULEB:
			...
			if skip, err := trie.ReadUleb128(r); err != nil {
				log.Panicf("io failed %v", err)
			} else {
				for i := uint64(0); i < count; i++ {
					var rebase types.Rebase
